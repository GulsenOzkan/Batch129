package day34mapsiterators;

import java.util.HashMap;
import java.util.TreeMap;

public class TreeMap01 {
    /*
        1-) TreeMap entry'leri ""key"'lere gore "natural order" a koyar.
        2-) TreeMap cok yavastir cunku natural order yapmak cok zaman alir.
        3-) TreeMap "thread-safe" ve "synchronize" degildir.
        4-) TreeMap'lerin "key"lerinde "null" kullanilamaz. Ama "value"larda istediginiz kadar kullanabilirsiniz.
     */
    public static void main(String[] args) {

        long one = System.nanoTime();
        TreeMap<String, Integer> stdAges = new TreeMap<>();
        stdAges.put("Ali", 21);
        stdAges.put("Kemal", 19);
        stdAges.put("Ayse", 25);
        stdAges.put("Murat", 22);

        // "key" lere gore alfabetik siralama yapti
        System.out.println(stdAges); // {Ali=21, Ayse=25, Kemal=19, Murat=22}

        long two = System.nanoTime();

        HashMap<String, Integer> stdAges2 = new HashMap<>();
        stdAges2.put("Ali", 21);
        stdAges2.put("Kemal", 19);
        stdAges2.put("Ayse", 25);
        stdAges2.put("Murat", 22);

        System.out.println(stdAges2);

        long three = System.nanoTime();
        System.out.println("TreeMap : " + (two - one)); // TreeMap : 852700
        System.out.println("HashMap: " + (three - two)); // HashMap: 61100

        HashMap<String, Integer> stdAges3 = new HashMap<>();
        stdAges3.put("Ali", 21);
        stdAges3.put("Kemal", 19);
        stdAges3.put("Ayse", 25);
        stdAges3.put("Murat", 22);

        TreeMap<String, Integer> stdAges4 = new TreeMap<>();

        System.out.println(stdAges4);

        long four = System.nanoTime();

        System.out.println("TreeMap : " + (two - one)); // TreeMap : 808900
        System.out.println("HashMap: " + (three - two)); // HashMap: 73200
        System.out.println("HashMap --> TreeMap: " + (four - three)); // HashMap --> TreeMap: 11616500


    }
}
